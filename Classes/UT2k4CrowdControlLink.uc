class UT2k4CrowdControlLink extends TcpLink transient;
...
simulated function handleMessage(string msg) {
    ...
    if (isCrowdControl(msg)) {
        ...
        Level.Game.Broadcast(self,"Got a weird message: "$msg);
    }
...
event Closed(){
    Level.Game.Broadcast(self,"Crowd Control connection closed");
    ListenPort = 0;
    reconnectTimer = ReconDefault;
}
...
function Resolved( IpAddr Addr )
{
    if (ListenPort == 0) {
        ListenPort=BindPort();
        if (ListenPort==0){
            Level.Game.Broadcast(self,"Failed to bind port for Crowd Control");
            reconnectTimer = ReconDefault;
            return;
        }   
    }
    ...
    if (False==Open(Addr)){
        Level.Game.Broadcast(self,"Could not connect to Crowd Control client");
        reconnectTimer = ReconDefault;
        return;
